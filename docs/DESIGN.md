...
| 候选生成 | `diaevo/generator.py` | 从挖掘簇渲染有证据支撑的 `SKILL.md` 草稿；可选生成受限 helper code、`code_artifacts.json` 和 `validation.json`。 |
...
当前默认演化目标是结构化 `SKILL.md` 文本。Phase 7 允许候选 skill 附带受限 helper code，但只作为候选制品在沙盒中验证，不直接进入生产代码。
