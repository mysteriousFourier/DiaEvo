...
tool_events
  ...
  -> review-script for code-backed skill
  -> queue-promotion
...
- 轨迹 ingest 和 mining。
- 候选 `SKILL.md` 生成。
- 可选 code-backed skill 候选：`generate --with-code` 生成只读 `scripts/skill_flow.py`、`code_artifacts.json` 和 `validation.json`，仍需 verifier、sandbox validation、`review-script` 和人工 promotion。
- 本地 metric/Pareto 演化。
- verifier 安全检查。
...
- `review-script` 可独立标注 helper 脚本为 `pending`、`approved` 或 `rejected`；脚本未 approved 或 validation 未 passed 时，推荐和使用应回退到 `SKILL.md`。
- promotion 仍然显式人工审批。
...
- 不提供 patch 且显式开启 `--collect-baseline` 时，命令会在 disposable sandbox 中运行验证命令，收集 stdout/stderr/exit code/touched files/diff，作为后续 patch guidance 输入。
...
1. 技能文本。
2. validation 元数据建议。
3. code-backed skill 的只读 helper code 候选，独立脚本审查，未审查时回退到 `SKILL.md`。
4. 自然语言 patch strategy。
5. 沙盒内 baseline 证据收集。
6. 沙盒内代码 patch。
7. 人工审查后显式应用到真实工作区。
...
```text
baseline sandbox
  -> patch / code-backed skill candidates
  -> cheap gates
  -> correctness sandbox
  -> maintainability gate
  -> optional efficiency benchmark
  -> Pareto report
  -> human review
```
...
1. 可执行性：先运行 baseline，再验证候选是否通过 build/test/lint/typecheck。不能复现 baseline 的任务不进入自动 patch 选择。
2. 安全性：路径白名单、危险命令、凭据、网络、安装命令和 helper capability 检查必须先于 sandbox 执行。
3. 可维护性：限制 touched files、diff 大小、函数长度、复杂度和模块边界；注释只要求解释非显然逻辑，不以“注释越多越好”为目标。
4. 效率：只有存在稳定 benchmark 时启用；多次运行取中位数，并设置最小提升阈值。没有 benchmark 时，不能把“性能优化”作为采纳理由。
5. 多目标选择：correctness、security、maintainability、performance、diff size 和 sandbox cost 进入 Pareto frontier；只把少量候选交给人工 review。
...
- `stop_if_security_warning = true`
- `human_review_required = always`

前沿依据：self-repair 研究显示迭代修复有效但主要收益集中在前几轮；self-improving coding agent 需要 reflection、benchmark 和代码更新记录；安全 benchmark 显示功能正确不等于安全；architecture smell benchmark 显示大范围重构可能引入新 smell。因此 DiaEvo 不做无限循环自进化，不在缺少 benchmark 时自动采纳性能优化，不自动合并真实代码。

CUA 类 sandbox 的定位：`trycua/cua` 适合 GUI、浏览器、桌面或移动手势类 skill 的隔离 replay；普通 repo code 和 skill-local helper code 仍默认使用 DiaEvo disposable workspace sandbox。

当前 code-backed skill 的脚本生成边界：

- 已支持 `generate --with-code` 从挖掘簇生成协同 skill 文件夹。
- 生成的脚本 v1 只做只读 flow 描述，不写 workspace、不运行 shell、不联网、不安装依赖。
- `validate --approve` 会把 validation 摘要写回 `code_artifacts.json`。
- `recommend` 会暴露 `script_available`、`script_status`、`execution_mode` 和 `fallback_reason`。
- 尚未实现从任意已有 `SKILL.md` 反向生成脚本；该能力应作为后续 `generate-script --skill <dir>` 增量设计。
