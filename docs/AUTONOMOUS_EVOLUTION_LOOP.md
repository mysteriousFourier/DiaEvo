...
- 轨迹 ingest 和 mining。
- 候选 `SKILL.md` 生成。
- 可选 code-backed skill 候选：`generate --with-code` 生成受限 helper code、`code_artifacts.json` 和 `validation.json`，仍需 verifier、sandbox validation 和人工 promotion。
- 本地 metric/Pareto 演化。
- verifier 安全检查。
...
1. 技能文本。
2. validation 元数据建议。
3. code-backed skill 的只读 helper code 候选。
