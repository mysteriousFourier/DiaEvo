...
1. Confirm the task matches the activation rule.
2. Inspect the smallest file set indicated by the trace evidence.
3. Run or emulate the recurring tool path: `chat_completion` -> `write_html` -> `edge_headless_screenshot` -> `rubric_evaluation` -> `glm_vision_evaluation`.
4. Apply the narrowest workspace-scoped change.
5. Record validation output and feed it back into DiaEvo.
