...
FLOW = {
  ...
  "workflow_steps": [
    ...
    "运行或模拟反复出现的工具路径：`chat_completion` -> `write_html` -> `edge_headless_screenshot` -> `rubric_evaluation` -> `glm_vision_evaluation`。",
    "如果任务出现 `视觉评估反馈`, `视觉评估反馈`, `截图硬门槛`，先复现失败场景，再开始编辑。",
